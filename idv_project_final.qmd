...
bins <- c(0, 10000, 20000, 40000, 50000, 60000, 70000, 80000, 90000, Inf)
pal <- colorBin("YlOrRd", domain = c(0, 100000), bins = bins)

labels <- sprintf(
  "<strong>%s</strong><br/> euros / m<sup>2</sup>",
...
m <- leaflet(geo_data) %>%
  ...
  addPolygons(
    fillColor = ~pal(as.numeric(geo_data$postinumeroalue)),
    weight = 2,
    opacity = 1,
    color = "white",
    dashArray = "3",
...
m %>% addLegend(pal = pal, values = as.numeric(geo_data$postinumeroalue), opacity = 0.7, title = NULL,
